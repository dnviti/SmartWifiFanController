...
def get_all_repo_files(root_dir=".", extensions=RELEVANT_EXTENSIONS, exclude_dirs=EXCLUDE_DIRS, exclude_files=EXCLUDE_FILES):
    ...
    for dirpath, dirnames, filenames in os.walk(root_dir):
        ...
        # This is more efficient than checking every file path.
        ...
        for filename in filenames:
            if filename.lower().endswith(extensions) and filename not in exclude_files:
                ...
                # Normalize path for consistency (e.g., ./src/main.cpp)
...
def main():
    ...
    if files_to_analyze_input == "*":
        ...
        # In GitHub Actions, the current directory is the root of the checkout.
        # For local testing, ensure you run this script from the repo root.
        ...
        print("FILES_TO_ANALYZE environment variable not set or empty, and not '*'.")
        # Example for local testing if not set:
        # files_to_analyze = ["./test/test_fan_control/test_fan_control.cpp", "./src/config.h"]
        if not files_to_analyze: # Check if files_to_analyze is still empty after potential manual assignment
             print("No files specified for analysis. Exiting.")
             exit(0) # Exit if no files are provided
    ...
    for i, file_path in enumerate(files_to_analyze):
        ...
        try:
            ...
            # Limit file content size to avoid excessive token usage / costs
            # Adjust MAX_FILE_SIZE_CHARS as needed. E.g., 100k characters.
            ...
            if identified_issues is None:
                ...
                for issue_data in identified_issues:
                    ...
                    # Use issue_data.get('file_path', file_path) to get the path, then os.path.basename
                    ...
                    # Ensure the full file_path is in the body for clarity
